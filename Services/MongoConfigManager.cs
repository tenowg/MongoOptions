...
namespace MongoOptions.Services
{
    ...
    public class MongoConfigManager(IServiceProvider sp, IMemoryCache cache, MongoConfigurationOptions configuration) : IConfigManager
    {
        ...
        public async Task UpdateConfigAsync<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicParameterlessConstructor)] T, TMeta>(string name, T value, TMeta? metadata = null, IMongoLockScope? lockScope = null) 
            where T : class, IConfigFile, IOptionsMetadata<TMeta>
            where TMeta : class, IMetadataOptions
        {
            if (metadata != null)
            {
                try
                {
                    var meta = value.GetMetadata(metadata);
                    await UpdateConfigAsync(name, value, meta, lockScope);
                }
                catch (Exception ex)
                {
                    // evnetually a custom exception type here
                    throw;
                }
            }
            await UpdateConfigAsync(name, value, null, lockScope);
        }
        ...
        public async Task<List<string>> GetKeysBuilder<T, TMeta>(
            FilterDefinition<ConfigDocument<T>> filter)
            where T : class, IConfigFile, IOptionsMetadata<TMeta>
        {
            var connection = sp.GetService<IMongoConnection<T>>();
            ...
            //var test = await connection.Collection.FindAsync<ConfigDocument<T>>(filter);
